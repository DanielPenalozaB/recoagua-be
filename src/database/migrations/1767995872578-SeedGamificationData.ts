...
export class SeedGamificationData1767995872578 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const challenges = await queryRunner.query(
      `SELECT id FROM "challenges" WHERE name = 'Calculadora'`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DELETE FROM "levels" WHERE name IN ('Novato acuático', 'Water Guardian', 'Maestro del agua')`,
    );
    await queryRunner.query(
      `DELETE FROM "challenges" WHERE name = 'Calculadora'`,
    );
    await queryRunner.query(
      `DELETE FROM "badges" WHERE name IN ('Primer bloque', 'Primer cálculo', 'Salvador del agua')`,
    );
  }
}
